...
namespace RTSEngine
{
    ...
    public class SelectedEntities
    {
        ...
        public virtual bool Add (List<Entity> entitiesList) //select a list of entities
        {
        ...
        public void Remove (List<Entity> entitiesList) { foreach(Entity e in entitiesList) { Remove(e); } }
        public void Remove (Entity entity)
        {
            if (selectedDic.TryGetValue(entity.GetCode(), out List<Entity> selectedList)) //try to find the dictionary entry for the entity's type
            {
                ...
                else if (Count == 1) //if only one entity is left selected, assign it as the single selected entity
