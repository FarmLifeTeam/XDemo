...
namespace RTSEngine
{
    public class MovementManager : MonoBehaviour
    {
        ...
        //a method that locally launches a non movement attack:
        public void LaunchNonMovementAttackLocal (Unit unit, FactionEntity targetEntity, bool playerCommand)
        {
            unit.AttackComp.SetTargetLocal(targetEntity, targetEntity.transform.position); //set the target

            if (!unit.AttackComp.IsTargetInRange()) //target not in range, do not proceed
            {
                if (playerCommand && unit.FactionID == GameManager.PlayerFactionID)
                    ErrorMessageHandler.OnErrorMessage(ErrorMessage.attackTargetOutOfRange, unit);
            }
            else
                unit.MovementComp.UpdateRotationTarget(targetEntity.transform.position, targetEntity.transform); //target in range, face it
        }
        ...
        public bool CanProceedAttackMovement (Unit unit, bool playerCommand, bool assignedAttackOnly, bool canChangeTarget, FactionEntity targetEntity)
        {
            ...
            {
                if (playerCommand == true && unit.FactionID == GameManager.PlayerFactionID) //player notification message
                    ErrorMessageHandler.OnErrorMessage(ErrorMessage.canNotAttack, unit);
